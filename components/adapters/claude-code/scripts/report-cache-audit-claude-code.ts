...
import {
  readRecentClaudeCodeCacheAuditRecords,
  summarizeClaudeCodeCacheAudit,
} from "../src/cache-audit.js";

async function main() {
  const tokenPilotConfigPath = process.env.TOKENPILOT_CLAUDE_CODE_CONFIG ?? defaultTokenPilotClaudeCodeConfigPath();
  const config = await loadTokenPilotClaudeCodeConfig(tokenPilotConfigPath);
  const records = await readRecentClaudeCodeCacheAuditRecords(config.stateDir, 64);
  if (records.length === 0) {
    console.log("TokenPilot Claude Code cache audit: no records yet.");
    return;
  }
  const summary = summarizeClaudeCodeCacheAudit(records);
  console.log("TokenPilot Claude Code cache audit report:");
