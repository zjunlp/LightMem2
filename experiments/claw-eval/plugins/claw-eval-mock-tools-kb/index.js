// @ts-nocheck
/**
 * Per-service shim: re-exports only the kb tools from claw-eval-mock-tools.
 */
import allToolsPlugin from "../claw-eval-mock-tools/index.js";

export default {
  id: "claw-eval-mock-tools-kb",
  name: "Claw-Eval Mock Tools: kb",
  description: "Registers only the kb mock service tools as OpenClaw tools.",

  register(api) {
    if (typeof api.registerTool !== "function") {
      if (api.logger) api.logger.warn("[claw-eval-mock-tools-kb] registerTool unavailable.");
      return;
    }
    ...
    if (allToolsPlugin && typeof allToolsPlugin.register === "function") {
      ...
      if (api.logger) api.logger.warn("[claw-eval-mock-tools-kb] unexpected main plugin export shape.");
      return;
    }

    if (api.logger) api.logger.info("[claw-eval-mock-tools-kb] kb tools registered.");
  },
};
